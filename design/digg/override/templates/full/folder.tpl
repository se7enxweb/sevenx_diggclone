...
<li id="main-submit"><a onclick="document.getElementById('btn-submit').click(); return false;" href="https://digg.one/submit">navigate to a category and submit a new story</a></li>
...
<a href="https://digg.one/rss/feed/digg" class="rss">front page stories in rss<img src={"digg/rss.gif"|ezimage()} alt="RSS" width="29" height="14"></a>
</div>
	...
	{if eq( ezini( 'folder', 'SummaryInFullView', 'content.ini' ), 'enabled' )}
        ...
        {if $node.object.data_map.show_children.data_int}
            {def $fetch_limit = 260
	         $page_limit = 10
                 $classes = array( 'link','category','article', 'folder' )
                 $children = array()
                 $children_count = ''}
                 
            {if le( $node.depth, '3')}
                {set $classes=array( 'link','category','article', 'folder' )}
            {/if}
...
{*    <b>{$index}</b> *}
    ...
    {foreach $childNodes as $child}
         ...
         {node_view_gui view=line content_node=$child[1] diggs=$child[0]}
         {delimiter}
