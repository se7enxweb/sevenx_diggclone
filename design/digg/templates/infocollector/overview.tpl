...
<h1 class="context-title">{'Digg spy (%object_count)'|i18n( 'design/admin/infocollector/overview',, hash( '%object_count', $object_count ) )}</h1>
...
{if $Objects.item.class_name|eq( 'Story' )}
...
*}
{def $story_node=fetch(content, node, hash(node_id, $Objects.item.main_node_id))
     $story_diggs=0
     $story_spy_link=concat( '/spy/collectionlist/', $Objects.item.contentobject_id)}
{node_view_gui view=line content_node=$story_node spy_link=$story_spy_link}
</td>
{/if}
</tr>
{/section}
